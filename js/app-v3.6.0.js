...
function treeNodeLabel(person){
  const summary = person.datos_resumen || person.rol || "";
...
function buildTree(focusId=treeFocusId){
  ...
  if(context){
    const parts = [
      ...
      `${siblings.length} ${siblings.length===1?"hermano":"hermanos"}`,
...
function fitTreeToView(){
  ...
  const scale = Math.min(1,Math.max(.18,Math.min(scaleX,scaleY)));
  transform = {
