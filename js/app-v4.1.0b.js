...
function canonicalParentIds(person){
  if(!person)return[];
  ...
  return uniqueExistingPersonIds([...direct,...legacy,...inverse])
    .filter(id=>id!==person.id);
}

function canonicalChildIds(person){
  if(!person)return[];
  ...
  const inverse=people
    .filter(candidate=>canonicalParentIds(candidate).includes(person.id))
    .map(candidate=>candidate.id);

  return uniqueExistingPersonIds([...direct,...inverse])
    .filter(id=>id!==person.id);
}

function canonicalSpouseIds(person){
  if(!person)return[];
...
function canonicalSiblingIds(person){
  if(!person)return[];

  const parentIds=canonicalParentIds(person);
  const explicit=[
    ...(Array.isArray(person.hermanos)?person.hermanos:[]),
    ...(Array.isArray(person.hermanas)?person.hermanas:[])
  ];

  const derived=parentIds.length
    ? people
        .filter(candidate=>candidate.id!==person.id)
        .filter(candidate=>canonicalParentIds(candidate).some(id=>parentIds.includes(id)))
        .map(candidate=>candidate.id)
    : [];

  return uniqueExistingPersonIds([...explicit,...derived])
    .filter(id=>id!==person.id);
}

/* Alias de compatibilidad. Toda la aplicación debe leer relaciones a través de ellos. */
function parentIdsFor(person){return canonicalParentIds(person)}
function childIdsFor(person){return canonicalChildIds(person)}
function spouseIdsFor(person){return canonicalSpouseIds(person)}
function siblingGroups(person){
  return canonicalSiblingIds(person)
    .map(id=>byId[id])
    .filter(Boolean);
}
...
function renderFamily(person){
  ...
  return `<div class="family-block">
    ...
    ${familyGroup("Cónyuges", person.conyuges, "No consta cónyuge")}
    ${familyGroup("Hijos", person.hijos, "No constan hijos")}
  </div>`;
}
