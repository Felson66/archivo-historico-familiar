...
function uniqueRelationIds(ids){
  const result=[];
  const seen=new Set();
  (Array.isArray(ids)?ids:[ids]).flat(Infinity).forEach(value=>{
...
function normalizedParentIds(person){
  if(!person)return[];
  const direct=[person.padre,person.madre];
  const legacy=Array.isArray(person.padres)?person.padres:[];
  const inverse=people
    .filter(candidate=>{
      const children=[
        ...(Array.isArray(candidate.hijos)?candidate.hijos:[]),
        ...(Array.isArray(candidate.hijas)?candidate.hijas:[])
      ];
      return children.includes(person.id);
    })
    .map(candidate=>candidate.id);

  return uniqueRelationIds([...direct,...legacy,...inverse])
    .filter(id=>id!==person.id);
}

function normalizedChildIds(person){
  if(!person)return[];
  const direct=[
    ...(Array.isArray(person.hijos)?person.hijos:[]),
    ...(Array.isArray(person.hijas)?person.hijas:[])
  ];
  const inverse=people
    .filter(candidate=>normalizedParentIds(candidate).includes(person.id))
    .map(candidate=>candidate.id);

  return uniqueRelationIds([...direct,...inverse])
    .filter(id=>id!==person.id);
}

function normalizedSpouseIds(person){
  if(!person)return[];
  const direct=[
    ...(Array.isArray(person.conyuges)?person.conyuges:[]),
    ...(Array.isArray(person.conyuge)?person.conyuge:[person.conyuge])
  ];
  const inverse=people
    .filter(candidate=>{
      const candidateSpouses=[
        ...(Array.isArray(candidate.conyuges)?candidate.conyuges:[]),
        ...(Array.isArray(candidate.conyuge)?candidate.conyuge:[candidate.conyuge])
      ];
      return candidateSpouses.includes(person.id);
    })
    .map(candidate=>candidate.id);
...
function normalizedSiblingIds(person){
  if(!person)return[];
  const parents=normalizedParentIds(person);
  const explicit=[
    ...(Array.isArray(person.hermanos)?person.hermanos:[]),
    ...(Array.isArray(person.hermanas)?person.hermanas:[])
  ];
  const derived=parents.length
    ? people
        .filter(candidate=>candidate.id!==person.id)
        .filter(candidate=>normalizedParentIds(candidate).some(id=>parents.includes(id)))
        .map(candidate=>candidate.id)
    : [];

  return uniqueRelationIds([...explicit,...derived])
    .filter(id=>id!==person.id);
}
...
function relationshipIntegrityReport(){
  ...
  people.forEach(person=>{
    const parents=normalizedParentIds(person);
    const spouses=normalizedSpouseIds(person);

    if(parents.includes(person.id)){
      issues.push({personId:person.id,type:"self-parent"});
    }

    if(spouses.includes(person.id)){
      issues.push({personId:person.id,type:"self-spouse"});
    }

    spouses.forEach(spouseId=>{
      const spouse=people.find(candidate=>candidate.id===spouseId);
      if(spouse&&!normalizedSpouseIds(spouse).includes(person.id)){
        issues.push({personId:person.id,relatedId:spouseId,type:"non-reciprocal-spouse"});
      }
    });
  });
