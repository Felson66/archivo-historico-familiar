...
function siblingIds(person){
  const parentIds = new Set((person.padres || []).filter(Boolean));
  if(!parentIds.size) return [];

  return PEOPLE
    .filter(candidate => candidate.id !== person.id)
    .filter(candidate => (candidate.padres || []).some(parentId => parentIds.has(parentId)))
    .sort((a,b) => a.nombre.localeCompare(b.nombre,"es"))
...
function renderFamily(person){
  const siblings = siblingIds(person);
  return `<div class="family-block">
    ${familyGroup("Padres", person.padres, "No constan padres registrados")}
    ${familyGroup("Hermanos", siblings, (person.padres || []).length ? "No constan otros hijos de sus padres" : "No se pueden calcular sin padres registrados")}
