...
const bioName=$("bioName"),bioBirthDate=$("bioBirthDate"),bioBirthPlace=$("bioBirthPlace"),bioDeathDate=$("bioDeathDate"),bioDeathPlace=$("bioDeathPlace"),bioProfession=$("bioProfession"),bioRole=$("bioRole"),bioState=$("bioState"),bioSummary=$("bioSummary"),savePersonalData=$("savePersonalData");
const factsEditorList=$("factsEditorList"),addFact=$("addFact"),saveFacts=$("saveFacts"),biographyResult=$("biographyResult");
...
function renderBiographyPanel(person){
  ...
  bioRole.value=person.rol||"";
  bioState.value=normalizeStateValue(person.estado);
  bioSummary.value=person.datos_resumen||"";
...
async function renderManager(){
  ...
  const person=currentPerson();
  if(!person){personSummary.classList.add("hidden");photoFraming.classList.add("hidden");biographyPanel.classList.add("hidden");photosCount.textContent="0";documentsCount.textContent="0";factsCount.textContent="0";galleryEmpty.textContent="Selecciona una persona para gestionar su archivo.";galleryEmpty.classList.remove("hidden");return}
...
savePersonalData.addEventListener("click",async()=>{
  ...
  try{
    ...
    await renderManager();
    showResult(biographyResult,"Datos personales guardados correctamente.");
  }catch(err){
...
saveFacts.addEventListener("click",async()=>{
  ...
  try{
    ...
    await writePeople();
    await renderManager();
    showResult(biographyResult,"Biografía y evidencias guardadas correctamente.");
  }catch(err){
