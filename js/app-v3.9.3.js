...
function parentIdsFor(person){
  ...
  // Compatibilidad con relaciones guardadas solo desde la ficha del progenitor.
  PEOPLE.forEach(candidate => {
    if((candidate.hijos || []).includes(person.id)) ids.add(candidate.id);
  });
...
function siblingGroups(person){
  ...
  PEOPLE.forEach(candidate => {
    ...
    if(shared.length >= 2){
      ...
      // Existe un progenitor común, pero faltan datos para afirmar que sean medios hermanos.
      commonParent.push(candidate.id);
    }
  });

  const sortIds = ids => ids.sort((a,b) => byId[a].nombre.localeCompare(byId[b].nombre,"es"));
  return {
    full: sortIds([...new Set(full)]),
    half: sortIds([...new Set(half)]),
    commonParent: sortIds([...new Set(commonParent)])
  };
}
  ...
  return `<div class="document-list">${docs.map(doc=>{
    ...
    return `<button type="button" class="document-card" data-document-url="1" data-document-id="${esc(doc.id)}" data-person-id="${esc(person.id)}">
      ...
      <div class="document-info">
        ...
        <span class="document-open">${String(doc.tipo||"").toLowerCase()==="txt"||String(doc.archivo||doc.src||doc.url||"").toLowerCase().endsWith(".txt")?"Leer documento":pageCount?`Consultar ${pageCount} página${pageCount===1?"":"s"}`:"Preparar vista desde Administración"} <span aria-hidden="true">→</span></span>
      </div>
    </button>`;
  }).join("")}</div>`;
}
...
async function openTextDocumentViewer(doc){
  ...
  viewer.className="text-document-viewer";
  viewer.innerHTML=`
    <header class="text-viewer-header">
      <div>
        <h2>${esc(doc.titulo||doc.nombre_archivo||"Documento de texto")}</h2>
        ${doc.descripcion?`<p>${esc(doc.descripcion)}</p>`:""}
      </div>
      <button class="text-viewer-close" type="button" aria-label="Cerrar documento">×</button>
    </header>
    <main class="text-viewer-body"><div class="text-viewer-loading">Cargando documento…</div></main>`;
  document.body.appendChild(viewer);
  document.body.classList.add("document-viewer-open");
  const close=()=>{viewer.remove();document.body.classList.remove("document-viewer-open")};
  viewer.querySelector(".text-viewer-close").addEventListener("click",close);
  try{
    const text=await fetchTextDocument(doc);
    viewer.querySelector(".text-viewer-body").innerHTML=`<pre class="text-document-content">${esc(text)}</pre>`;
  }catch(error){
    viewer.querySelector(".text-viewer-body").innerHTML=`<div class="text-viewer-error"><strong>No se pudo abrir el documento de texto.</strong><span>${esc(error.message)}</span></div>`;
  }
}
