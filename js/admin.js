...
function relationIds(value){
  ...
  values.flat(Infinity).forEach(raw=>{
    ...
    if(!id||seen.has(id)||!people.some(person=>person.id===id))return;
    seen.add(id);
    result.push(id);
  });
...
function parentIdsForAdmin(person){
  if(!person)return[];

  const ids=new Set([
    ...relationIds(person.padre),
    ...relationIds(person.madre),
    ...relationIds(person.padres)
  ]);

  people.forEach(candidate=>{
    const children=[
      ...relationIds(candidate.hijos),
      ...relationIds(candidate.hijas)
    ];
    if(children.includes(person.id))ids.add(candidate.id);
  });
...
function childIdsForAdmin(person){
  if(!person)return[];

  const ids=new Set([
    ...relationIds(person.hijos),
    ...relationIds(person.hijas)
  ]);

  people.forEach(candidate=>{
    if(parentIdsForAdmin(candidate).includes(person.id))ids.add(candidate.id);
  });
...
function spouseIdsForAdmin(person){
  if(!person)return[];

  const ids=new Set([
    ...relationIds(person.conyuges),
    ...relationIds(person.conyuge)
  ]);

  people.forEach(candidate=>{
    const spouses=[
      ...relationIds(candidate.conyuges),
      ...relationIds(candidate.conyuge)
    ];
    if(spouses.includes(person.id))ids.add(candidate.id);
  });
...
function relationshipIntegrityReport(){
  const issues=[];

  people.forEach(person=>{
    const parents=parentIdsForAdmin(person);
    const spouses=spouseIdsForAdmin(person);

    if(
      String(person.padre||"").trim()===person.id ||
      String(person.madre||"").trim()===person.id
    ){
      issues.push({type:"self-parent",personId:person.id});
    }
    ...
    if(rawSpouses.includes(person.id)){
      issues.push({type:"self-spouse",personId:person.id});
    }
    ...
    spouses.forEach(spouseId=>{
      ...
      const spouseRaw=[
        ...relationIds(spouse.conyuges),
        ...relationIds(spouse.conyuge)
      ];
      if(!spouseRaw.includes(person.id)){
        issues.push({
          type:"non-reciprocal-spouse",
          personId:person.id,
          relatedId:spouseId
        });
      }
    ...
    parents.forEach(parentId=>{
      if(childIdsForAdmin(person).includes(parentId)){
        issues.push({
          type:"possible-cycle",
          personId:person.id,
          relatedId:parentId
...
function renderWorkspaceVisibility(tab){
  ...
  biographyPanel.classList.toggle("hidden",!showBiography);

  if(showPhotos){
    photoFraming.classList.toggle("hidden",!currentPerson()?.fotografia_principal);
  }
}
...
async function renderPhotoFraming(person){
  ...
  photoFraming.classList.toggle("hidden",activeTab!=="photos");
}
...
function legacyBirthData(person){
  const result={date:"",place:""};
  const summary=String(person?.datos_resumen||"").trim();
  if(!summary)return result;

  const parts=summary.split("·").map(part=>part.trim()).filter(Boolean);
  if(!parts.length)return result;

  const deathIndex=parts.findIndex(part=>/^[†✝]/.test(part));
  const birthParts=deathIndex>=0?parts.slice(0,deathIndex):parts;

  if(birthParts[0] && /\b\d{1,2}\b.*\b(1[5-9]\d{2}|20\d{2}|21\d{2})\b/i.test(birthParts[0])){
    result.date=birthParts[0];
  }

  if(birthParts[1] && !/^[†✝]/.test(birthParts[1])){
    result.place=birthParts[1];
  }

  return result;
}

function structuredBirthDate(person){
  return String(person?.fecha_nacimiento||"").trim() || legacyBirthData(person).date;
}

function structuredBirthPlace(person){
  return String(person?.lugar_nacimiento||"").trim() || legacyBirthData(person).place;
}

function renderBiographyPanel(person){
  if(!person)return;
  bioName.value=person.nombre||"";
  bioBirthDate.value=structuredBirthDate(person);
  bioBirthPlace.value=structuredBirthPlace(person);
...
function expedientLifeText(person){const b=extractYear(structuredBirthDate(person)),d=extractYear(person.fecha_defuncion);if(b&&d)return `${b} – ${d}`;if(b&&person.situacion_vital==="vivo")return `${b} –`;if(b)return `n. ${b}`;if(d)return `† ${d}`;if(person.situacion_vital==="vivo")return "Persona viva";if(person.situacion_vital==="fallecido")return "Persona fallecida";return "Fechas no documentadas"}
function renderExpedientHeader(person){
  ...
  const meta=[structuredBirthPlace(person),person.profesion].filter(Boolean).join(" · ");
...
async function renderManager(){
  ...
  if(!person){
    ...
    photosEmpty.textContent="Selecciona una persona para gestionar su archivo.";
  ...
  documentsCount.textContent=docs.length;
  ...
  documentsEmpty.classList.toggle("hidden",docs.length>0);

  photos.forEach((photo,index)=>{
    const principal=person.fotografia_principal===photo.src;
    const linked=new Set(photo.personas||[person.id]).size;
